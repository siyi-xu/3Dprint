    ...
    for poly in polygons:  # 遍历多边形
        for i in range(poly.count() - 1):  # 遍历多边形顶点
            ...
            if v1.crossProduct(v2).dz < 0:  # 判断当前顶点是否是凹顶点
                a = radToDeg(v1.getAngle2D())  # 向量v1和X轴夹角a，0~360度
                b = radToDeg(v2.getAngle2D())  # 向量v2和X轴夹角b，0~360度
                a, b = min(a, b), max(a, b)  # 强制使：a<b
                anglePairs.append((a, b))  # 添加夹角对(a,b)到anglePairs
    return anglePairs


def _initAngleTable(digit):  # 角度表构建函数，digit为小数位数
    angleTable = {}  # 角度字典
    delta = math.pow(10, -digit)  # 角度增量
    angle = 0.0
    while angle < 180:  # 在0~180度范围内，创建字典键

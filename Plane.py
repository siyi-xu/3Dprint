...
class Plane:
    def __init__(self, P, N):
        self.P = P.clone()  # P为平面经过的一点
        self.N = N.clone().normalized()  # N为平面法向量,单位向量

    def __str__(self):
        return "Plane\n%s\n%s\n" % (str(self.P), str(self.N))

    def toFormula(self):  # 获取标准直线方程Ax+By+Cz+D=0 中参数A,B,C,D值
    ...
    def intersect(self, other):  # 两平面求交,返回Line会None
        dir = self.N.crossProduct(other.N)  # dir 为直线方向向量
        if dir.isZeroVector():
            return None
        else:
            x, y, z = 0, 0, 0  # x,y,z为直线经过点坐标
            A1, B1, C1, D1 = self.toFormula()
            A2, B2, C2, D2 = other.toFormula()  # 通过平面标准方程联立求解
            if B2 * C1 - B1 * C2 != 0:

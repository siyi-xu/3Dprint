...
# 对填充区域进行分区，以保证在整体上能够使用 Z 形对填充线进行连接，用于方向平行填充
class SplitRegion:
    def __init__(self, polygons, adjustPolyDirs=False):  # 初始化函数
        ...
        self.splitPolygons = self.split()  # 分区结果存储在splitPolygons中

    def split(self):  # 略过split函数实现
        ...
        if len(turnPts) != 0:  # 如果凹峰点个数不为0，则继续
            ...
            if len(splitters) != 0:  # 如果切分矩形个数不为0，继续
                ...
                clipper.AddPaths(ca.toPaths(self.polygons), PT_SUBJECT)
                clipper.AddPaths(ca.toPaths(splitters), PT_CLIP)
                sln = clipper.Execute(CT_DIFFERENCE)  # 布尔差运算
                return ca.toPolys(sln, turnPts[0].z)  # 返回裁剪结果
        return self.polygons

    def findTurnPoints(self):  # 凹峰点寻找函数
        vx = Vector3D(1, 0, 0)  # 定义和X轴平行的向量vx
    ...
    def findLRPoints(self, pt, ptses):  # 寻找凹峰点两侧交点函数
        for pts in ptses:  # 遍历二维点列表第一个维度
            if len(pts) > 0 and pts[0].y == pt.y:  # 定位高度相等的层
                for i in range(len(pts) - 1):  # 遍历列表第二个维度
                    if pt.x > pts[i].x and pt.x < pts[i + 1].x:  # 通过x坐标定位凹峰点两侧点
                        return pts[i], pts[i + 1]  # 返回找到的两侧点
        return None, None  # 如果没找到，则返回None

    def createSplitter(self, p1, p2, delta=1.0e-4):  # 创建切分矩形
        vx, vy = Vector3D(1, 0, 0), Vector3D(0, 1, 0)  # 创建沿X、Y轴的单位向量
        splitter = Polyline()  # 切分矩形实际为封闭多段线
...
def splitRegion(polygons, adjustPolyDirs=False):  # 定义SplitRegion类的接口函数
